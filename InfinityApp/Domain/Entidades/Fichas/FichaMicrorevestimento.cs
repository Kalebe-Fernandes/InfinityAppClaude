...
/// <summary>
/// Representa uma ficha de serviço de Microrevestimento.
/// </summary>
public class FichaMicrorevestimento : FichaBase
{
    public virtual ICollection<ApontamentoMicrorevestimento> Apontamentos { get; set; } = [];

    public void AdicionarApontamento(ApontamentoMicrorevestimento apontamento)
    {
        if (!PodeSerEditada())
            throw new InvalidOperationException("Não é possível adicionar apontamentos a uma ficha que não está pendente.");

        apontamento.FichaMicrorevestimentoId = Id;
