...
/// <summary>
/// Representa uma ficha de serviço de Fresagem.
/// </summary>
public class FichaFresagem : FichaBase
{
    public virtual ICollection<ApontamentoFresagem> Apontamentos { get; set; } = [];

    public void AdicionarApontamento(ApontamentoFresagem apontamento)
    {
        if (!PodeSerEditada())
            throw new InvalidOperationException("Não é possível adicionar apontamentos a uma ficha que não está pendente.");

        apontamento.FichaFresagemId = Id;
